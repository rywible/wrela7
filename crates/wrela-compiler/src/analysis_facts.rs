            ...
            let semantic_fields =
                facts
                    ...
                    .and_then(|record| match &record.kind {
                        SemanticTypeKind::Structure {
                            arguments, fields, ..
                        } if arguments.is_empty() && !fields.is_empty() => Some(fields),
                        _ => None,
                    });
            semantic_fields.is_some_and(|semantic_fields| {
                let value_has_type =
                ...
                let bool_value = |value: wrela_sema::ValueId| {
                    facts
                        .values
                        .get(value.0 as usize)
                        .filter(|record| record.function == expression.function)
                        .and_then(|record| facts.types.get(record.ty.0 as usize))
                ...
                matches!(
                    ...
                    && derived_fields.len() == semantic_fields.len()
                    && conjunctions.len() == semantic_fields.len().saturating_sub(1)
                    && derived_fields.iter().zip(semantic_fields).enumerate().all(
                        |(index, (derived, semantic))| {
                            u32::try_from(index) == Ok(derived.field)
                                && is_stored_runtime_scalar(facts, semantic.ty)
                                && value_has_type(derived.left, semantic.ty)
                                && value_has_type(derived.right, semantic.ty)
                                && bool_value(derived.comparison)
                        },
                    )
                    && conjunctions.iter().all(|value| bool_value(*value))
            })
        }
