...
use wrela_driver::{
    Command, CommandOutput, CompilerDriver, DoctorCheck, DoctorOutcome, DriverError, TestSelection,
};
use wrela_package::ImageDeclaration;
...
use wrela_toolchain::{
    ComponentKind, Toolchain, ToolchainDecodeLimits, ToolchainManifestCodec, VerifiedToolchain,
};
...
pub fn run(command: &Command) -> Result<CommandOutput, DriverError> {
    match command {
        Command::Doctor => {
            ...
            let checks = toolchain
                .doctor()
                .checks
                .into_iter()
                .map(|check| {
                    DoctorCheck::new(check.name.to_owned(), check.path, check.present).map_err(
                        |_| {
        ...
        _ => Err(DriverError::CompilerServicesUnavailable),
    }
}
...
pub trait AnalysisFactAssembler {
    ...
    ) -> Result<wrela_image_report::ValidatedAnalysisFacts, wrela_image_report::ReportError>;
}
...
enum FilePublicationPayload {
    Bytes(Vec<u8>),
    TestReport(Box<wrela_test_model::EncodedTestReport>),
}
...
impl TestReportPublicationRequest {
    ...
    pub fn report(&self) -> &wrela_test_model::ValidatedTestReport {
        match &self.file.payload {
            FilePublicationPayload::TestReport(encoded) => encoded.report(),
            FilePublicationPayload::Bytes(_) => {
                unreachable!("test report publication always retains its encoded report")
            }
        }
    }
}
...
pub fn seal_test_report_publication_request(
    ...
    let file = seal_file_publication_payload(
        hasher,
        destination,
        FilePublicationPayload::TestReport(Box::new(encoded)),
        FilePublicationMode::CreateNew,
        maximum_bytes,
        is_cancelled,
    )?;
    Ok(TestReportPublicationRequest { file })
}
...
pub fn seal_test_report_publication(
    ...
    let file = seal_file_publication(&request.file, receipt)?;
    let encoded = match request.file.payload {
        FilePublicationPayload::TestReport(encoded) => *encoded,
        FilePublicationPayload::Bytes(_) => {
            return Err(HostError::PublicationMismatch(file.path().to_owned()));
        }
    };
...
pub struct BuildPlanningRequest<'a> {
    ...
    pub standard_library_digest: Sha256Digest,
}
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct PlannedBuild(ValidatedBuildConfiguration);
...
pub fn seal_build_plan(
    ...
    let canonical_request = canonical_build_request(request)?;
    ...
    if selected_image != Some(request.image)
        ...
        || identity.standard_library != request.standard_library_digest
...
fn canonical_build_request(
    ...
    push_request_string(&mut bytes, &request.image.name)?;
    push_request_string(&mut bytes, &request.image.module.dotted())?;
    push_request_string(&mut bytes, &request.image.entry)?;
    push_request_string(&mut bytes, request.target.identity().as_str())?;
    push_request_string(&mut bytes, &request.profile.name)?;
    match request.intent {
        BuildIntent::Check => bytes.push(0),
        BuildIntent::Build => bytes.push(1),
        BuildIntent::Test { selection } => {
            bytes.push(2);
            match selection {
                TestSelection::All => bytes.push(0),
                TestSelection::Comptime => bytes.push(1),
                TestSelection::Integration => bytes.push(2),
                TestSelection::Images => bytes.push(3),
                TestSelection::NameContains(value) => {
                    ...
                    bytes.push(4);
                    push_request_string(&mut bytes, value)?;
                }
            }
        }
    }
...
fn push_request_string(output: &mut Vec<u8>, value: &str) -> Result<(), BuildPlanningError> {
    let length = u32::try_from(value.len()).map_err(|_| {
        BuildPlanningError::Selection("canonical request field exceeds u32 bytes".to_owned())
    })?;
