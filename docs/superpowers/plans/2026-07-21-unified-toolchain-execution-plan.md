...
| — | General `match`/`is` over ADTs (consumer; uses T0.1, needed for AdmissionResult consumption) | T0.1 | **A-1 plus bounded A3 alternatives landed** — mixed-arity/per-variant-type exhaustive statement match; guarded constructor arms, payload `_`, and a final whole-pattern `_` with exact coverage and sealing; unit/payload-wildcard `is`; and `return match` tails. An unguarded arm may join distinct payload-free constructors of the exact scrutinee enum; every alternative contributes exact exhaustiveness coverage (`runtime_adt_match_unit_alternatives_contribute_exact_exhaustive_coverage`). One such group lowers as a single canonical default after explicit arms cover its exact complement, with no duplicated body SSA; terminal scalar matches reach deterministic native COFF (`one_unit_pattern_alternative_group_lowers_as_exact_complement_default`, `unit_pattern_alternative_group_reaches_flow_machine_and_deterministic_native_coff`). Analysis also admits one shared payload local when every alternative exposes the same exact payload type, with one sealed semantic value/definition (`runtime_adt_match_alternatives_share_one_exact_payload_binding`); mixed types/shapes fail closed and lowering stops at `semantic-match-alternative-payload-lowering-pending`. Multiple alternative groups, a source wildcard beside alternatives, and guarded fallback overlap have named lowering boundaries (`multiple_pattern_alternative_groups_stay_named_fail_closed`, `pattern_alternative_default_composition_tails_fail_closed_by_name`). General value-producing match expressions, broader executable alternatives, executable payload-binding alternatives, fixed-array patterns, and success-dominated `is` binding remain absent or named fail-closed. |
