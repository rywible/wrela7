...
| 2 | actor-state initialization/storage floor | L2.1a | **native storage complete in this slice** — exactly `value: u64 = 0` synthesizes an actor-owned eight-byte `.state` image region with a dense capacity proof and exact HIR sealing through SemanticWir v9 / FlowWir v10. MachineWir v12 adds a closed `ActorState { actor }` kind, authenticates mailbox→state→turn ordering plus common actor stem/owner/source/proof/layout/zero initialization, and reaches the generic writable-global LLVM path with byte-identical repeat ARM64 COFF (`actor_state_schema_revision_accepts_only_exact_machine_wir_v12`, `canonical_zero_actor_state_reaches_native_machine_storage`). Machine output equality now seals `region_storage` exactly and cancellably. Multiple/richer/app state layouts stay fail-closed; receiver-address wiring and `self.field` access/mutation remain absent. |
