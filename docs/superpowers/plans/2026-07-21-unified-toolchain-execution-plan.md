...
| — | General `match`/`is` over ADTs (consumer; uses T0.1, needed for AdmissionResult consumption) | T0.1 | **A-1 plus bounded A3 alternatives landed** — mixed-arity/per-variant-type exhaustive statement match; guarded constructor arms, payload `_`, and a final whole-pattern `_` with exact coverage and sealing; unit/payload-wildcard `is`; and `return match` tails. An unguarded analysis-tier arm may now join distinct payload-free constructors of the exact scrutinee enum; every alternative contributes exact exhaustiveness coverage (`runtime_adt_match_unit_alternatives_contribute_exact_exhaustive_coverage`). Duplicate, guarded, and payload-bearing alternatives remain named fail-closed, and lowering stops at `semantic-match-alternatives-lowering-pending` (`runtime_adt_match_alternative_tails_fail_closed_by_name`, `unit_pattern_alternatives_stop_at_named_semantic_lowering_boundary`). Existing guarded/tail uniform-unary subsets still reach deterministic native COFF. General value-producing match expressions, executable alternatives, payload-binding alternatives, fixed-array patterns, and success-dominated `is` binding remain absent or named fail-closed. |
