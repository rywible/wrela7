...
| 2 | L2.5 scope-op lowering (B4b/c) | L2.2,B4a | **L2.5a normal path plus direct-return cleanup reach the authenticated native boundary** — parameterless free-call scopes with literal flat state and pass-only exit cross SemanticWir→FlowWir as authenticated `GeneratedCleanup` calls. Machine lowering and LLVM independently admit only the exact helper/proof/state contract and render its aggregate ABI deterministically (`authenticated_normal_scope_cleanup_carries_flat_state_to_machine_wir`, `authenticated_normal_cleanup_emits_deterministic_native_coff`). The first abnormal-exit increment moves a direct unit return behind `CommitScope`/`ExitScope`; directly nested scopes clean inner-before-outer, a missing exit fails sealing, Flow retains the cleanup call immediately before `Return`, and exact operation/cancellation limits are pinned (`early_return_from_free_scope_runs_cleanup_before_return`, `nested_free_scope_return_cleans_inner_before_outer`, `early_return_scope_lowering_has_exact_limit_and_late_cancellation`). Structured returns remain named `semantic-with-structured-return-cleanup-lowering-pending`; parameterized acquisition, abort, break/continue, `?`/failure/await, non-pass cleanup, and event observation remain named fail-closed. |
