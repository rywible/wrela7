...
| 0 | T0.1 deferred tails (fail-closed) | T0.1 | one fixed nongeneric flat-structure payload inside the admitted primitive-scalar generic-enum specialization now reaches exact SemanticWir; Flow stops at `flow-enum-nominal-payload-lowering-pending`. Nongeneric nominal enum-payload lowering, fixed enum payloads, and generic/view/tuple/array payloads remain named-diagnostic fail-closed. |
