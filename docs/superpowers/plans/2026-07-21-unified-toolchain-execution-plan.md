...
| 3 | B6a–b immediate typed async delivery | L2.2,L2.4 | **constant and one bounded computed internal-async profile landed** — one actor turn and one single-slot task may each immediately await the same no-argument ordinary async helper. Besides the prior unit/constant-`u64` bodies, the helper may call one exact synchronous constant-`u64` seed, define one constant RHS, perform one unsigned checked add, and return the sum. FlowWir v14 retains that body plus its strict-linear token and resume value; Machine lowering, MachineWir v15, and LLVM independently authenticate the seed/call/checked-add/result chain while erasing only token/resume plumbing. `parsed_actor_source_delivers_exact_u64_computed_async_results_to_actor_and_task` covers source execution, extra-tail/operand/argument/result/seed forgeries, exact instruction N/N−1 bounds, late cancellation, codec, repeat Machine equality, and deterministic native COFF. Multiple awaits/states, parking/wake, cancellation execution, arbitrary argumented/computed helpers, broader results, recursion, and a general scheduler remain fail-closed. |
