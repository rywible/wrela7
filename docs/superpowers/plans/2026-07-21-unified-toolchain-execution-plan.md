...
| A7a-d | Reserved multiline syntax + static literals + bounded primitive interpolation | **extended through analysis-only bool and primitive-integer interpolation** — `"""` remains one recoverable reserved token. Unannotated, unconsumed synchronous locals retain exact compiler-minted `Static[Str]` / `Static[Bytes[N]]` literal types; bool/integer interpolation mints a reclaimable-linear `BoundedString[capacity]`. Capacity is checked decoded UTF-8 bytes plus five per bool, `digits(2^N−1)` per unsigned integer, or a sign plus `digits(2^(N−1))` per signed integer; `usize`/`isize` use the selected target width. Exact source-order witnesses retain each integer expression, value, semantic type, and maximum decimal width. `bounded_integer_interpolation_uses_exact_primitive_decimal_widths` covers all fixed widths, target-sized widths, default `i64` literals, and interning; `bounded_integer_interpolation_full_seal_rejects_type_width_and_value_forgery` authenticates the new witness. Bool and integer N/N−1 type bounds and last-poll cancellation are pinned separately. Format specs, nonprimitive and broader computed expressions, ISR/async use, explicit contexts, and all consumers stay named fail-closed (`bounded_primitive_interpolation_tails_fail_closed_by_name`). Semantic lowering remains stopped at `semantic-bounded-string-lowering-pending` (`bounded_primitive_interpolation_stops_at_named_semantic_lowering_boundary`). General `Format`, owned string/bytes operations, SemanticWir/ABI storage, and native lowering remain queued. |
