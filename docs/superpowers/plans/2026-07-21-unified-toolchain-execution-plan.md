...
| 2 | L2.5 scope-op lowering (B4b/c) | L2.2,B4a | **L2.5a normal-path-only Flow cleanup landed in this slice** — parameterless free-call scopes with literal flat state and pass-only exit lower in actor turns to an exact activation-specific `ScopePlan` and marker sequence, then cross SemanticWir→FlowWir as one authenticated `GeneratedCleanup` specialization and a real state-carrying call per normal exit. The Flow sealer independently regenerates the functions/calls and rejects scope-origin or cleanup erasure forgery; the report counts exact cleanup edges (`pass_only_scope_lowers_to_authenticated_normal_cleanup_call`). Nested Flow calls preserve proved inner-before-outer order and reversed dependency metadata fails closed (`nested_scope_cleanup_calls_follow_proved_inner_before_outer_order`); exact SemanticWir sealing, N/N−1 operation limits, and late cancellation remain covered. Parameterized/nested state acquisition, abort/return/`?`/failure/await, non-pass cleanup, machine/native scope authentication, and event observation remain named fail-closed (`scope_abnormal_and_non_pass_tails_remain_named_fail_closed`). |
