...
- **L2.2 — Non-scalar values through WIR + codegen.** Aggregates/views/replies
  represented (or erased) through SemanticWir v11 / FlowWir v13 / MachineWir v14 +
...
| 2 | L2.5 scope-op lowering (B4b/c) | L2.2,B4a | **L2.5a normal/direct-return paths reach the authenticated native boundary; the first structured-return path reaches FlowWir.** Parameterless free-call scopes with literal flat state and pass-only exit cross SemanticWir→FlowWir as authenticated `GeneratedCleanup` calls, and the existing exact helper/proof/state contract reaches deterministic MachineWir/LLVM. Direct unit returns clean directly nested scopes inner-before-outer before `Return`. A single-arm, no-`else` conditional return now inserts cleanup on both taken-return and fallthrough paths, with path-sensitive Flow validation/lowering, forged-path sealing, exact operation/block/instruction/depth limits, and late cancellation (`structured_scope_return_cleans_return_and_fallthrough_paths`, `structured_scope_return_lowers_cleanup_on_both_cfg_paths`, `nested_structured_scope_return_has_exact_depth_boundary`). This structured increment stops at FlowWir because Machine lowering still fail-closes branched async activation callers; no native claim is made for it. `if`/`else`, loop/match exits, parameterized acquisition, abort, break/continue, `?`/failure/await, non-pass cleanup, and event observation remain named fail-closed. |
