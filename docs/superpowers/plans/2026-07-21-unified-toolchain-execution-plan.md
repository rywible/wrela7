...
| 0 | T0.3 generics/monomorphization (A6) | T0.1 | **T0.3a–T0.3e landed** — flat structures, module-level synchronous functions, and closed enums specialize over ordered primitive stored-copy-scalar type arguments. Generic enum construction is expected-type-directed and its exact specialization survives `match`/`is`; full sealing re-derives arguments, substituted variants, tagged-union layout, and the separate authenticated core `Result` contract (`generic_enum_specializes_copy_scalar_payload_for_construction_match_and_is`, `generic_enum_nominal_payload_tail_fails_closed_by_name`). Function inference combines expected results and already-typed arguments, repeat instances deduplicate, signatures substitute exactly, recursive specialization is bounded by an active stack, and full sealing independently derives the specialization key/signature (`generic_copy_scalar_function_specializes_expected_types`, `generic_function_infers_from_a_typed_argument_without_result_context`). The concrete method substrate resolves unique visible inherent and concrete-impl nongeneric synchronous `read self` calls and seals receiver/target/visibility exactly. A uniform unary generic enum whose specialized payload is one identical copy scalar now lowers through exact actor SemanticWir construction/match operations (`uniform_copy_scalar_generic_enum_specialization_lowers`); core `Result` authentication remains distinct, while mixed-arity and heterogeneous specializations retain named boundaries. Const/bounded/region parameters, nested nominal/view arguments, nominal generic-enum payloads, unconstrained/conflicting inference, recursion, generic structures/functions, generic/async methods, mut/take/temporary receivers, associated calls, generic interfaces/bounds, and native generic-enum proof remain fail-closed. Existing parsed/HIR `region` generics remain unchanged pending the source-spec/normative-fixture reconciliation. |
