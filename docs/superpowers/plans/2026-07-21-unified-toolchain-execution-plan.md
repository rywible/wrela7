...
| B4a | `with`/scope sema analysis: cleanup DAG + `CleanupAcyclic` + cycle diagnostic | **landed**; parameterless/pass-only actor-turn scopes also reach the exact SemanticWir normal-path precursor |
...
| 1 | B4a cleanup DAG sema analysis | — | **landed** 43d3e279 — free-call scope protocols/calls, lexical activations, reverse-source cleanup DAG + `CleanupAcyclic`, synthetic cycle detector, and named await/receiver/outside-`with` rejections. The bounded L2.5a precursor now lowers parameterless, pass-only actor-turn activations to exact SemanticWir `ScopePlan` records, deterministic `Cleanup` helpers, and ordered normal-path `EnterScope`/`CommitScope`/`ExitScope` markers (`free_scope_normal_exit_lowers_exact_semantic_cleanup_sequence`, `nested_free_scopes_preserve_proved_reverse_cleanup_order`). Flow execution remains named fail-closed. |
...
| 0 | T0.3 generics/monomorphization (A6) | T0.1 | **T0.3a–T0.3h plus heterogeneous SemanticWir lowering landed** — flat structures, module-level synchronous functions, concrete synchronous `read self` methods on nongeneric flat structures, and closed enums specialize over ordered primitive stored-copy-scalar type arguments. Generic enum construction is expected-type-directed and its exact specialization survives `match`/`is`; full sealing re-derives arguments, substituted variants, tagged-union layout, and the separate authenticated core `Result` contract (`generic_enum_specializes_copy_scalar_payload_for_construction_match_and_is`, `generic_enum_nominal_payload_tail_fails_closed_by_name`). Function and method inference combine expected results and already-typed explicit arguments, repeat instances deduplicate, signatures substitute exactly, recursive specialization is bounded by an active stack, and full sealing independently derives the specialization key/signature plus the method receiver/target/visibility (`generic_copy_scalar_function_specializes_expected_types`, `generic_function_infers_from_a_typed_argument_without_result_context`, `generic_read_receiver_method_specializes_and_seals_exactly`). A nongeneric interface requirement and its concrete impl method may declare alpha-equivalent ordered unbounded method-owned type parameters (`concrete_impl_generic_method_specializes`). Unary generic enums with uniform or per-variant heterogeneous copy-scalar specializations and primitive stored-copy-scalar generic flat structures lower through exact actor SemanticWir construction/match/projection operations; heterogeneous enum layout uses the widest payload and the match binding retains each variant's exact type (`heterogeneous_copy_scalar_generic_enum_specialization_lowers_exactly`). Module-level synchronous generic copy-scalar function instances retain exact specialized parameters/results, body returns, instance keys, and direct-call targets in actor SemanticWir; lowering independently authenticates module ownership, ordered unbounded HIR type parameters, concrete scalar arguments, and every signature substitution, while sealing rejects key and target forgery (`generic_copy_scalar_function_specializations_lower_exactly`, `generic_function_lowering_tails_fail_closed_by_name`). Generic method lowering remains named fail-closed. Core `Result` authentication remains distinct; mixed-arity generic enums and non-type/non-scalar structure/function/method arguments retain named boundaries. Const/bounded/region parameters, nested nominal/view arguments, nominal generic-enum payloads, unconstrained/conflicting inference, recursion, generic receiver structures, generic structure field updates, async methods, mut/take/temporary receivers, associated calls, generic interfaces/bounds, Flow/native heterogeneous-enum execution, generated-test proof, and native generic proof remain fail-closed or unclaimed. Existing parsed/HIR `region` generics remain unchanged pending the source-spec/normative-fixture reconciliation. |
...
| 2 | L2.5 scope-op lowering (B4b/c) | L2.2,B4a | **L2.5a SemanticWir precursor landed in this slice** — parameterless free-call scopes with literal flat state and pass-only exit lower in actor turns to an exact activation-specific `ScopePlan`, authenticated `Cleanup` helper, and normal marker sequence. Nested scopes preserve the proved inner-before-outer order; exact sealing, N/N−1 operation limits, and late cancellation are covered. Parameterized/nested state acquisition, abort/return/`?`/failure/await, non-pass cleanup, Flow cleanup calls, machine/native execution, and event observation remain named fail-closed; `flow-scope-cleanup-lowering-pending (normal-exit cleanup calls)` is the next sealed boundary. |
