...
| 3 | A5a one-field derived equality | T0.3,L2.2 | **landed in this slice** — `deriving(Eq)` on an exact nongeneric structure with one stored copy-scalar field gives `==` and `!=` real borrowed structural semantics. Sema records two authenticated field intermediates, independently seals deriving/nominal/type/value identity, and leaves both aggregate operands readable. Semantic lowering reauthenticates the source declaration and emits two read projections followed by the existing scalar comparison; FlowWir/MachineWir and LLVM require no schema extension. `one_field_derived_eq_reaches_deterministic_native_coff` proves canonical codec round-trip and repeat byte-identical ARM64 COFF; adjacent named tests cover missing deriving, unsupported shapes, field/intermediate forgeries, exact operation N/N−1, and late cancellation. A single multi-field enum payload is now correctly rejected for `deriving(From)`. Multi-field/generic/nested structures, enums, `Format`, and executable `From` remain fail-closed or unimplemented. |
...
| 3 | B6a–b immediate typed async delivery | L2.2,L2.4 | **constant and bounded two-link computed internal-async profiles landed** — one actor turn and one single-slot task may each immediately await the same no-argument ordinary async helper. Besides the prior unit/constant-`u64` bodies, the helper may call one exact synchronous constant-`u64` seed, define one constant RHS, and perform a canonical chain of one or two unsigned checked additions before returning the final sum. FlowWir v14 retains that body plus its strict-linear token and resume value; Machine lowering, MachineWir v15, and LLVM independently authenticate every seed/call/checked-add/result link while erasing only token/resume plumbing. `parsed_actor_source_delivers_exact_u64_computed_async_results_to_actor_and_task` covers source execution, extra-tail and per-link operand forgeries, argument/result/seed forgeries, exact instruction N/N−1 bounds, late cancellation, codec, repeat Machine equality, and deterministic native COFF. Longer chains, multiple awaits/states, parking/wake, cancellation execution, argumented helpers, broader results, recursion, and a general scheduler remain fail-closed. |
