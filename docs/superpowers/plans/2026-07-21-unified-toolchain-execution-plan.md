...
| A7a-c | Reserved multiline syntax + static literals + bounded bool interpolation | **extended through an analysis-only bool-interpolation increment** — `"""` remains one recoverable reserved token. Unannotated, unconsumed synchronous locals retain the exact compiler-minted `Static[Str]` / `Static[Bytes[N]]` literal types. In addition, interpolation with one-or-more core `bool` values and no format specifier mints a reclaimable-linear `BoundedString[capacity]`; capacity is the checked sum of decoded literal UTF-8 bytes and five maximum bytes per bool, so `f"ready={true}"` proves exactly 11. The resolution fact records every text fragment and bool expression/value in source order. Full sealing independently replays order, content, types, values, capacity, effects, ownership, and exact type interning (`bounded_bool_interpolation_full_seal_rejects_witness_forgeries`); exact type N/N−1 and last-poll cancellation are pinned by `bounded_bool_interpolation_type_budget_is_exact_and_cancels_at_the_last_poll`. Format specs, non-bool values, ISR/async use, explicit contexts, and all consumers stay named fail-closed (`bounded_bool_interpolation_tails_fail_closed_by_name`). Semantic lowering stops at the explicit `semantic-bounded-string-lowering-pending` boundary (`bounded_bool_interpolation_stops_at_named_semantic_lowering_boundary`). General `Format`, owned string/bytes operations, SemanticWir/ABI storage, and native lowering remain queued. |
...
| 3 | B6a–b immediate typed async delivery | L2.2,L2.4 | **bounded two-state sequential delivery landed** — one actor turn and one single-slot task may each perform exactly two sequential awaits of the same no-argument ordinary async helper. Each caller has one authenticated state 0 followed by one authenticated state 1; the first resume enters the second call block and the second enters an empty return block. Four distinct activation regions preserve per-site source/capacity authority with maximum-live one. The helper retains the prior exact synchronous seed plus one-or-two checked-add computation subset. FlowWir v14 retains every strict-linear token and resume value; Machine lowering, MachineWir v15, and LLVM independently authenticate the complete ordered chain while erasing only token/resume plumbing. `parsed_actor_source_delivers_exact_u64_computed_async_results_to_actor_and_task` now covers duplicate-state and skipped-middle forgeries, exact two-state N/N−1 bounds, existing helper/result/proof forgeries, exact instruction bounds, cancellation, codec, repeat Machine equality, and deterministic native COFF. More than two awaits, non-sequential suspension CFGs, parking/wake, cancellation execution, argumented helpers, broader results, recursion, and a general scheduler remain fail-closed. |
