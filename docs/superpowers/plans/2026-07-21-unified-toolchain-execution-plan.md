...
| A7a-e | Reserved multiline syntax + static literals + bounded primitive interpolation | **extended through analysis-only bool, character-literal, and primitive-integer interpolation** — `"""` remains one recoverable reserved token. Unannotated, unconsumed synchronous locals retain exact compiler-minted `Static[Str]` / `Static[Bytes[N]]` literal types; admitted interpolation mints a reclaimable-linear `BoundedString[capacity]`. Capacity is checked decoded UTF-8 bytes plus five per bool, four per character, `digits(2^N−1)` per unsigned integer, or a sign plus `digits(2^(N−1))` per signed integer; `usize`/`isize` use the selected target width. Exact source-order witnesses retain character expression/value identity and each integer's semantic type and maximum decimal width. `bounded_character_interpolation_uses_exact_utf8_width` proves the four-byte bound and mixed literal-text capacity; `bounded_character_interpolation_full_seal_rejects_witness_forgery` authenticates expression/value identity; exact N/N−1 type bounds and last-poll cancellation are pinned separately. Character support is literal-only; ordinary character locals/expressions, format specs, floats, nonprimitive and broader computed expressions, ISR/async use, explicit contexts, and all consumers stay named fail-closed (`bounded_primitive_interpolation_tails_fail_closed_by_name`). Semantic lowering remains stopped at `semantic-bounded-string-lowering-pending` (`bounded_primitive_interpolation_stops_at_named_semantic_lowering_boundary`). General `Format`, owned string/bytes operations, SemanticWir/ABI storage, and native lowering remain queued. |
...
- **L2.2 — Non-scalar values through WIR + codegen.** Aggregates/views/replies
  represented (or erased) through SemanticWir v12 / FlowWir v15 / MachineWir v15 +
