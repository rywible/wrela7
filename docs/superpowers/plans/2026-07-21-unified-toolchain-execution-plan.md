...
| 0 | T0.3 generics/monomorphization (A6) | T0.1 | **T0.3a landed in this slice** — analysis interns type-only generic flat structures specialized by ordered primitive stored-copy-scalar arguments; expected types drive bare construction, field reads use substituted types, repeat specializations deduplicate, and full sealing re-authenticates exact HIR fields plus independently derived primitive/specialized layouts (`generic_flat_structure_specializes_copy_scalar_type_arguments`). Const/bounded/region, nested-nominal, and unconstrained-inference tails remain named fail-closed (`generic_flat_structure_rejects_constant_parameters_by_name`, `generic_flat_structure_rejects_nominal_type_arguments_by_name`, `generic_flat_structure_constructor_requires_expected_specialization`); generic functions/interfaces are subsequent increments, and lowering rejects `semantic-generic-structure-lowering-pending`. Existing parsed/HIR `region` generics remain unchanged pending the source-spec/normative-fixture reconciliation. |
