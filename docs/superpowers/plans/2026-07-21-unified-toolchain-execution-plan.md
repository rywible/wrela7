...
| 0 | T0.3 generics/monomorphization (A6) | T0.1 | **T0.3a–T0.3g landed** — flat structures, module-level synchronous functions, and closed enums specialize over ordered primitive stored-copy-scalar type arguments. Generic enum construction is expected-type-directed and its exact specialization survives `match`/`is`; full sealing re-derives arguments, substituted variants, tagged-union layout, and the separate authenticated core `Result` contract (`generic_enum_specializes_copy_scalar_payload_for_construction_match_and_is`, `generic_enum_nominal_payload_tail_fails_closed_by_name`). Function inference combines expected results and already-typed arguments, repeat instances deduplicate, signatures substitute exactly, recursive specialization is bounded by an active stack, and full sealing independently derives the specialization key/signature (`generic_copy_scalar_function_specializes_expected_types`, `generic_function_infers_from_a_typed_argument_without_result_context`). The concrete method substrate resolves unique visible inherent and concrete-impl nongeneric synchronous `read self` calls and seals receiver/target/visibility exactly. A uniform unary generic enum and primitive stored-copy-scalar generic flat structure lower through exact actor SemanticWir construction/match/projection operations. Module-level synchronous generic copy-scalar function instances now also retain exact specialized parameters/results, body returns, instance keys, and direct-call targets in actor SemanticWir; lowering independently authenticates module ownership, ordered unbounded HIR type parameters, concrete scalar arguments, and every signature substitution, while sealing rejects key and target forgery (`generic_copy_scalar_function_specializations_lower_exactly`, `generic_function_lowering_tails_fail_closed_by_name`). Core `Result` authentication remains distinct, while mixed-arity/heterogeneous enums and non-type/non-scalar structure/function arguments retain named boundaries. Const/bounded/region parameters, nested nominal/view arguments, nominal generic-enum payloads, unconstrained/conflicting inference, recursion, generic structure field updates, generic/async methods, mut/take/temporary receivers, associated calls, generic interfaces/bounds, generated-test proof, and native generic proof remain fail-closed or unclaimed. Existing parsed/HIR `region` generics remain unchanged pending the source-spec/normative-fixture reconciliation. |
