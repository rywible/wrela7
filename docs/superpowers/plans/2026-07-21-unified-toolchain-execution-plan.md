...
| 0 | T0.3 generics/monomorphization (A6) | T0.1 | **T0.3a + T0.3b + T0.3c analysis tiers landed** — flat structures and module-level synchronous functions specialize over ordered primitive stored-copy-scalar type arguments. Function inference combines expected results and already-typed arguments, repeat instances deduplicate, signatures substitute exactly, recursive specialization is bounded by an active stack, and full sealing independently derives the specialization key/signature (`generic_copy_scalar_function_specializes_expected_types`, `generic_function_infers_from_a_typed_argument_without_result_context`). The concrete method substrate resolves unique visible inherent and concrete-impl nongeneric synchronous `read self` calls on named flat-structure values, binds explicit arguments after the receiver with the established label rule, and seals receiver/target/visibility exactly (`concrete_read_receiver_method_call_is_exactly_sealed`, `concrete_interface_impl_method_call_analyzes`, `method_call_tails_fail_closed_by_name`). Const/bounded/region and nested-nominal arguments, unconstrained/conflicting inference, recursion, generic/async methods, mut/take/temporary receivers, and associated calls remain named fail-closed. Lowering rejects `semantic-generic-structure-lowering-pending`, `semantic-generic-function-lowering-pending`, and `semantic-method-call-lowering-pending`. Existing parsed/HIR `region` generics remain unchanged pending the source-spec/normative-fixture reconciliation. |
