...
| 0 | T0.3 generics/monomorphization (A6) | T0.1 | **T0.3a–T0.3d analysis tiers landed** — flat structures, module-level synchronous functions, and closed enums specialize over ordered primitive stored-copy-scalar type arguments. Generic enum construction is expected-type-directed and its exact specialization survives `match`/`is`; full sealing re-derives arguments, substituted variants, tagged-union layout, and the separate authenticated core `Result` contract (`generic_enum_specializes_copy_scalar_payload_for_construction_match_and_is`, `generic_enum_nominal_payload_tail_fails_closed_by_name`). Function inference combines expected results and already-typed arguments, repeat instances deduplicate, signatures substitute exactly, recursive specialization is bounded by an active stack, and full sealing independently derives the specialization key/signature (`generic_copy_scalar_function_specializes_expected_types`, `generic_function_infers_from_a_typed_argument_without_result_context`). The concrete method substrate resolves unique visible inherent and concrete-impl nongeneric synchronous `read self` calls and seals receiver/target/visibility exactly. Const/bounded/region parameters, nested nominal/view arguments, nominal generic-enum payloads, unconstrained/conflicting inference, recursion, generic/async methods, mut/take/temporary receivers, associated calls, generic interfaces/bounds, and lowering remain named fail-closed. Existing parsed/HIR `region` generics remain unchanged pending the source-spec/normative-fixture reconciliation. |
...
| 2 | actor-state initialization/storage floor | L2.1a | **analysis through FlowWir landed in this slice** — exactly `value: u64 = 0` synthesizes an actor-owned eight-byte `.state` image region with a dense capacity proof, exact HIR sealing, and unchanged SemanticWir v9 / FlowWir v10 propagation (`zero_initialized_actor_state_has_exact_owned_storage`, `canonical_zero_actor_state_reaches_flow_and_named_machine_boundary`). MachineWir v11's closed storage-kind taxonomy has no actor-state identity, so the vertical fails closed at `machine-actor-state-storage-lowering-pending`; a versioned `ActorState` storage variant is the next lowering slice. Receiver-address wiring and `self.field` access/mutation remain absent. |
...
| 2 | L2.2 values-through-WIR + codegen | L2.1a,T0 | **L2.2a + L2.2b landed in this slice** — nongeneric flat locals with two or more scalar fields retain an aligned unpacked MachineWir v11 struct with explicit construct/copy/insert/extract operations, exact validation/equality/metering, and LLVM `insertvalue`/`extractvalue` rendering. Typed CFG edges and aggregate PHIs now preserve the value across branch joins; both the straight-line and joined verticals emit byte-identical repeated ARM64 COFF (`unpacked_struct_operations_validate_exact_field_types_and_layout`, `two_field_flat_local_reaches_unpacked_machine_wir_and_deterministic_coff`, `two_field_flat_local_crosses_cfg_join_and_emits_deterministically`). One-field-`u64` erasure remains byte-compatible. Aggregate function boundaries and non-flat shapes remain named fail-closed; views/replies remain pending. |
