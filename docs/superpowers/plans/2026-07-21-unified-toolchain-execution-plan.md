...
| A7a-f | Reserved multiline syntax + static literals + bounded interpolation | **extended through analysis-only exact static-text interpolation** — in addition to bool, character literals, and primitive integers, a direct `Static[Str]` literal or direct binding contributes its exact decoded UTF-8 extent to compiler-minted `BoundedString[capacity]`. The witness retains and seals source expression, semantic value, compiler-minted static type, part order, effects, and ownership (`bounded_static_text_interpolation_uses_exact_utf8_width`, `bounded_static_text_interpolation_full_seal_rejects_witness_forgery`). Exact semantic-type N/N−1 and final-poll cancellation remain pinned (`bounded_static_text_interpolation_type_budget_is_exact_and_cancels_at_the_last_poll`). This narrow operand path does not weaken the general `semantic-static-data-consumer-pending` boundary. `Static[Bytes[N]]` remains `semantic-bounded-interpolation-value-type`: chapter 10 defines no standard byte-format spelling. Format specs, floats, broader computed expressions, ISR/async use, explicit contexts, consumers, SemanticWir/ABI storage, and native lowering stay named fail-closed; static-text interpolation reaches `semantic-bounded-string-lowering-pending` (`bounded_static_text_interpolation_stops_at_named_semantic_lowering_boundary`). General `Format` and owned string/bytes operations remain queued. |
...
- **L2.2 — Non-scalar values through WIR + codegen.** Aggregates/views/replies
  represented (or erased) through SemanticWir v12 / FlowWir v15 / MachineWir v16 +
...
| 0 | T0.1 deferred tails (fail-closed) | T0.1 | one fixed nongeneric flat-structure payload inside the exact two-unary-variant primitive-scalar generic-enum specialization now reaches authenticated FlowWir v15 and its canonical wire frame. Machine lowering stops at `machine-enum-nominal-payload-lowering-pending`; other nominal enum-payload profiles retain `flow-enum-nominal-payload-lowering-pending`. Nongeneric nominal enum-payload semantic lowering, fixed enum payloads, and generic/view/tuple/array payloads remain named-diagnostic fail-closed. |
| 0 | · T0.3r fixed flat generic-enum payload through FlowWir | T0.3p | **landed in this slice** — the exact two-unary-variant profile (one fixed nongeneric flat structure plus one substituted primitive scalar) carries its Aggregate/MakeEnum operations through authenticated FlowWir v15 and canonical codec round-trip. The Flow validator rejects tag/payload identity substitution (`fixed_flat_generic_enum_payload_reaches_exact_flow_and_named_machine_boundary`, `fixed_flat_enum_payload_accepts_exact_nominal_operations_and_rejects_substitution`). Machine lowering stops at `machine-enum-nominal-payload-lowering-pending`; broader nominal Flow profiles retain `flow-enum-nominal-payload-lowering-pending`. |
| 0 | · T0.3s generic flat field replacement through native | T0.3q,L2.2 | **landed in this slice** — the exact two-field `Cell[u64]` owned-local replacement profile reaches canonical FlowWir v15, unpacked MachineWir v16, and repeat byte-identical native COFF. `generic_flat_field_update_reaches_flow_machine_and_deterministic_coff` proves field-bound forgery rejection, codec identity, construct/insert/extract identity, exact Flow and Machine instruction N/N−1 ceilings, and late cancellation at both lowerings. Other field counts/type arguments, compound/nested/nonlocal replacement, generic aggregate call boundaries, and runtime generic metadata remain unclaimed or fail closed. |
| 0 | · T0.3t asymmetric core Result analysis + SemanticWir | T0.3,A-1 | **landed in this slice** — exact public `core.result.Result[T,E]` accepts two distinct primitive stored-copy-scalar arguments. Expected-type-directed `Ok`/`Err`, exhaustive `match`/`is`, and conversion-free owned-rvalue `?` preserve each variant's exact payload type when the enclosing result is the identical specialization. The shared tagged slot uses maximum size/alignment. Analysis pins exact value N/N−1, final-poll cancellation, and full-seal Err-type forgery rejection; SemanticWir independently authenticates construction/match/propagation (`asymmetric_core_result_constructs_consumes_and_propagates_exact_payload_types`, `asymmetric_core_result_lowers_exact_payloads_and_question_propagation`). Non-scalar arguments, named-place propagation, error conversion/`From`, Flow/native heterogeneous payload execution, and general outcome-taxonomy composition remain unclaimed or fail closed. |
...
| 3 | feature verticals (B1b tails,B2b/c,B3,B5c tails,B6b–B9,A1/3/4/5/7/8) | tiers 0–2 | queued |
| 3 | B10 inferred placement specification | F5,B9 | **normative amendment landed; implementation queued** — Foundations §8.1 and Actors §15.1 now define a deterministic closed-world bin pack over sealed maximum-work and reserved-byte report facts, with explicit per-actor assignments fixed first, canonical actor/core tie breaks, hard-capacity rejection, report provenance, and build-identity sealing. The design ledger records inference-with-override as the approved evolution. No producer, two-core vertical, SPSC lowering, or multicore runtime is claimed. |
