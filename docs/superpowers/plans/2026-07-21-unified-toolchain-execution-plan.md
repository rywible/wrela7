...
| A7a | Reserved multiline-string syntax | **landed** — `"""` is consumed as one recoverable literal-shaped token and rejected once with `syntax-reserved-multiline-string` over the opening delimiter (`triple_quoted_string_is_one_exact_reserved_syntax_error`). Static/bounded string value types, byte arrays, `Format`, interpolation analysis, capacity proofs, and lowering remain queued. |
