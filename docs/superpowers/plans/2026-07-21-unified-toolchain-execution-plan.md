...
- Region ops (`Allocate`/`ResetRegion`/`Promote`) and scope ops
  ...
  lowering stage. `runtime.S` has no scheduler; the generated image-entry
...
- **L2.2 — Non-scalar values through WIR + codegen.** Aggregates/views/replies
  represented (or erased) through SemanticWir v11 / FlowWir v12 / MachineWir v13 +
...
| 2 | L2.4 reply-slot + scheduler (B5c) | L2.2,T0.1 | **bounded scheduler prerequisite landed in this slice** — one capacity-one mailbox admits exactly startup→first turn→distinct terminal second turn for same-actor unit messages. Sema proves and reauthenticates the finite chain; SemanticWir/FlowWir/MachineWir retain exactly two reserve/commit/receive edges; the machine and LLVM seals authenticate the same two distinct `SchedulerFifo` callers and one shared tag-switched image-entry drain. The native vertical pins cancellation/resource boundaries, rejects caller/permit/receive/dispatch forgeries, and emits repeat byte-identical ARM64 COFF (`capacity_one_startup_to_turn_chain_is_admitted_non_reentrantly`, `recurring_chain_rejects_non_capacity_one_and_cycles_stably`, `capacity_one_recurring_chain_drains_two_unit_turns_to_native_coff`). The existing 16-byte mailbox ABI is unchanged and reply-extensible. Reply slots/ABI, `ReplyResolve`, awaited replies, runtime admission, payloads, longer/cyclic/cross-actor recurrence, and the `runtime.S` per-core scheduler remain queued; this row is not complete. |
| 2 | L2.5 scope-op lowering (B4b/c) | L2.2,B4a | **L2.5a normal-path pass-only cleanup reaches native** — parameterless free-call scopes with literal flat state and pass-only exit cross SemanticWir→FlowWir as authenticated `GeneratedCleanup` calls. Machine lowering admits only the dense generated suffix paired with its exact source helper, identical pass-only body/state signature, and helper→activation `CleanupAcyclic` proof chain; the aggregate argument is legal only on that exact call. `authenticated_normal_scope_cleanup_carries_flat_state_to_machine_wir` covers forgery, N/N−1 function capacity, and late cancellation. The LLVM consumer independently reauthenticates the pair/call and renders the existing aggregate ABI deterministically (`authenticated_normal_cleanup_call_renders_exact_aggregate_llvm_boundary`; `authenticated_normal_cleanup_emits_deterministic_native_coff` under LLVM). Ordinary aggregate function boundaries remain fail-closed. Nested Flow calls preserve proved inner-before-outer order (`nested_scope_cleanup_calls_follow_proved_inner_before_outer_order`). Parameterized/nested state acquisition, abort/return/`?`/failure/await, non-pass cleanup, and event observation remain named fail-closed (`scope_abnormal_and_non_pass_tails_remain_named_fail_closed`). |
