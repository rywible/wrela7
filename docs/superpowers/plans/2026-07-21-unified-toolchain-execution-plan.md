...
| 1 | B1a view/provenance semantics | lexical provenance model | **B1a.5 asymmetric acyclic joins landed in this slice** — the regionless `LexicalView` authenticates canonical path-terminal references and exact sorted live-after statement IDs across straight-line, `if`/`else`, and exhaustive enum `match`. Backward joins conservatively OR path liveness, with an absent `else` contributing the continuation state, so a view may have a terminal use in only some arms while mutation or suspension remains permitted in an arm that never retains it. Branch-local release, source freeze, await, carrier rebound, and direct-root disjointness consult that structured witness. Full sealing recomputes both the view-level and statement-level witnesses from HIR and rejects a paired forgery that falsely marks the nonretaining arm live (`asymmetric_if_view_liveness_releases_the_nonretaining_branch`, `asymmetric_if_view_liveness_allows_nonretaining_await`, `asymmetric_match_view_liveness_uses_an_or_join`). The prior symmetric and continuation cases remain covered by `lexical_projection_terminal_uses_are_exact_across_if_branches`, `lexical_projection_terminal_uses_are_exact_across_match_arms`, and `lexical_projection_liveness_flows_to_post_if_continuation`. Loops, `with`, early exits, unused views, receiver/wrapped/generic projections, broader consumption, mutation through views, projected-path disjointness, iterator access, and lowering remain named fail-closed. |
