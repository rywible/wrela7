...
- **L2.2 — Non-scalar values through WIR + codegen.** Aggregates/views/replies
  represented (or erased) through SemanticWir v11 / FlowWir v12 / MachineWir v14 +
...
| 2 | L2.3 region/escape runtime lowering | L2.1b,L2.2 | **producer landed; runtime gap remains** — B2b emits exact semantic assignment/promotion facts, report rows, and `SemanticOperation::Promote` for direct actor-state stores; `semantic-promotion-lowering-pending` remains until FlowWir/runtime representation lands |
